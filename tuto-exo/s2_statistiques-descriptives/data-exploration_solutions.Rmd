...
```{r hist_log2_LFQ_altogether_pretty, fig.width=7, fig.height=4.5, out.width="100%", fig.cap="Histogram of log2(LFQ) values from Canale's whole proteomics dataset. "}
## Create a vector with all the numeric values of the proteome quantification
x <- na.omit(unlist(log2_LFQ))
...
- Actually,  we don't expect *a priori* that this curve would follow a normal distribution, or any particular distribution, because it represents values for >8000 proteins, each of which is expected to have a specific level (concentration) related to transcriptional and post-transcriptional regulation
...
```{r hist_with_parameters, fig.width=7, fig.height=4.5, out.width="80%", fig.cap="Histogramme des valeurs de protéome avec les paramètres de tendance centrale et de dispersion"}
### Draw a histogram ###
...
## Mark the median
abline(v = proteome_parameters$median, col = "darkgreen", lwd = 5, lty = "dotted")
