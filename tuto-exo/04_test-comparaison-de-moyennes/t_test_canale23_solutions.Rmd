...
## We multiply by (n-1)*n to obtain the sample variance instead of the "corrected" variance returned by var()
var_per_protein_liver <- apply(log2_LFQ_liver, 1, var, na.rm=TRUE) * ((n_liver -1 ) /n_liver)
var_per_protein_HCC <- apply(log2_LFQ_HCC, 1, var, na.rm=TRUE) * ((n_HCC -1 ) /n_HCC)
...
plot(var_per_protein_liver, 
     var_per_protein_HCC, 
     main = "Variance of the abundance per protein: Liver versus HCC",
...
## We multiply the sd() result by sqrt((n-1)/n) to obtain the sample standard deviation 
## instead of the "corrected" estimate of population standard deviation returned by sd()
...
HCC_values <- unlist(log2_LFQ[poi, HCC_samples])
liver_values <- unlist(log2_LFQ[poi, liver_samples])
...
#### Compute manually the Welch statistics ####
n1 <- sum(!is.na(HCC_values))
n2 <- sum(!is.na(liver_values))
s1_est <- sd(HCC_values, na.rm = TRUE)
s2_est <- sd(liver_values, na.rm = TRUE)
var1_est <- var(HCC_values, na.rm = TRUE)
var2_est <- var(liver_values, na.rm = TRUE)
m1_est <- mean(HCC_values, na.rm = TRUE)
m2_est <- mean(liver_values, na.rm = TRUE)

d <- m1_est - m2_est
SE <- sqrt(s1_est^2/n1 + s2_est^2/n2)

t_obs <- d / SE
df <- (var1_est / n1 + var2_est /n2 )^2 / (var1_est^2/(n1^2*(n1 - 1)) + var2_est^2/(n2^2 * (n2- 1)))
p <- pt(q = -abs(t_obs), df = df, lower.tail = TRUE) * 2
...
proteome_t$n1 <- apply(log2_LFQ[rownames(proteome_t), HCC_samples], 1, function(x) {sum(!is.na(x))})
proteome_t$n2 <- apply(log2_LFQ[rownames(proteome_t), liver_samples], 1, function(x) {sum(!is.na(x))})
# plot(proteome_t$n1, proteome_t$n2, col = densCols(proteome_t$n1, proteome_t$n2))

proteome_t$s1_est <- apply(log2_LFQ[rownames(proteome_t), HCC_samples], 1, sd, na.rm=TRUE)
proteome_t$s2_est <- apply(log2_LFQ[rownames(proteome_t), liver_samples], 1, sd, na.rm=TRUE)
proteome_t$SE <- sqrt(proteome_t$s1_est^2/proteome_t$n1 + proteome_t$s2_est^2/proteome_t$n2) ## Estimation of the standard error on the mean difference
proteome_t$d <- (proteome_t$xbar_1 - proteome_t$xbar_2) ## Effect size
proteome_t$t_check <- proteome_t$d / proteome_t$SE      ## t statistics recomputed

# proteome_t$p_shapiro_HCC <- apply(log2_LFQ[rownames(proteome_t), HCC_samples], 1, shapiro.test(), na.rm)
# proteome_t$p_shapiro_liver <- apply(log2_LFQ[rownames(proteome_t), liver_samples], 1, shapiro.test(), na.rm=TRUE)
...
write.table(proteome_t, 
            file = file.path(result_dir, "Canale23_proteome_differential_analysis.tsv"),
...
library(xlsx)
write.xlsx(x = proteome_t, file = file.path(result_dir, "Canale23_proteome_differential_analysis.xlsx"))

## system(paste("open", result_dir))
...
plot(proteome_t$M, 
     y, 
     main = "Welch test result – Volcano plot\nProteome data from Canale (2023)",
     xlab = "Effect size: d = liver - HCC",
     ylab = "-log10(P-value)",
...
****************************************************************
## Echantillons associés par paires

Les données protéomiques de Canale (2023) contiennent des échantillons associés par paire : les auteurs ont prélevé chez chaque patient un échantillon d'hépatocarcinome cellulaire et un échantillon de foie non cancéreux. L'association est cependant incomplète, pour certains patients on ne dispose que d'un des deux échantillons. 

```{r sample_pairing}

samples_per_patient <- table(sample_groups$sample)
paired_samples <- names(samples_per_patient[samples_per_patient == 2])
message("Number of patients with paired samples: ", length(paired_samples))

sample_groups_paired <- subset(x = sample_groups, sample %in% paired_samples)
# dim(sample_groups_paired)

sample_groups_paired_frame <- data.frame(
  HCC = paste0("HCC_", paired_samples),
  Liver = paste0("Liver_", paired_samples)
  )
rownames(sample_groups_paired_frame) <- paired_samples
sample_groups_paired_frame

log2_LFQ_paired <- log2_LFQ[, rownames(sample_groups_paired)]
# dim(log2_LFQ_paired)
# View(log2_LFQ_paired)



write.xlsx(x = log2_LFQ_paired, file = file.path(result_dir, "Canale23_proteome_paired_samples.xlsx"))

# ## Data filtering : filter out proteins having too few numeric values in one of the groups
# min_values_per_group <- 3
# 
# ## Count number of NA values per group for each protein
# n_HCC_per_prot_paired <- apply(!is.na(log2_LFQ_paired[, sample_groups_paired_frame$HCC]), 1, sum)
# n_liver_per_prot_paired <- apply(!is.na(log2_LFQ_paired[, sample_groups_paired_frame$Liver]), 1, sum)
# 
# # kable(table(n_HCC_per_prot > min_values_per_group, n_liver_per_prot > min_values_per_group))
# selected_proteins_paired <- (n_HCC_per_prot_paired  > min_values_per_group) & (n_liver_per_prot_paired  > min_values_per_group)
# # table(selected_proteins_paired)
# 
# 
# paired_cols <- split(rownames(sample_groups_paired),
#                      sample_groups_paired$sample)
# paired_cols <- paired_cols[sapply(paired_cols, length) == 2]
# 
# 
# 
# ## Define a function that runs a t test and returns the quantitative results as a vector.
# ## This enables to use it with apply() in order to run the t test on each row of a omics data table.
# my_t_paired_bof <- function (values, groups, ...) {
#   group_names <- unique(groups)
#   x <- values[groups == group_names[1]]
#   y <- values[groups == group_names[2]]
#   
# # t_result <- t.test(values ~ groups, ... )
#  t_result <- t.test(x, y, ... )
#  t_result_vector <- c(
...
# }
# 
# my_t_paired <- function(values, paired_cols, ...) {
# 
#   x <- sapply(paired_cols, function(cols) values[cols[1]])
#   y <- sapply(paired_cols, function(cols) values[cols[2]])
# 
...
#     p = t_result$p.value,
#     conf.int = t_result$conf.int,
#     mean_HCC   = mean(x, na.rm=TRUE),
#     mean_Liver = mean(y, na.rm=TRUE),
#     stderr = t_result$stderr,
#     M = mean(x - y, na.rm=TRUE),
#     A = (mean(x,na.rm=TRUE) + mean(y,na.rm=TRUE)) / 2
#   )
# }
# 
# # dim(log2_LFQ_paired[selected_proteins_paired, sample_groups_paired_frame$HCC])
# # dim(log2_LFQ_paired[selected_proteins_paired, sample_groups_paired_frame$Liver])
# # 
# # hist(apply(!is.na(log2_LFQ_paired[selected_proteins_paired, sample_groups_paired_frame$HCC]), 1, sum), breaks = 0:32)
# # hist(apply(!is.na(log2_LFQ_paired[selected_proteins_paired, sample_groups_paired_frame$Liver]), 1, sum), breaks = 0:32)
# 
# ## Compute a table with all the t-test results. 
# ## This returns a data frame with one row per protein and one column per statistics. 
# proteome_t <- as.data.frame(t(apply(
#   log2_LFQ_paired[selected_proteins_paired,], 
#   1, my_t_paired, 
#   unlist(sample_groups_paired$group),
#   alternative = "two.sided", 
#   paired = TRUE)))
# 
# proteome_t <- as.data.frame(t(
#   apply(log2_LFQ_paired[selected_proteins_paired,], 
#         1,
#         my_t_paired,
