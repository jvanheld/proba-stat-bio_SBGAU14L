...
for (n in n_values) {
  ...
  means_m <- mean(sample_means) # compute the mean of the sample means
  means_sd <- sd(sample_means) # compute the standard deviation of the sample means
  means_var <- var(sample_means) # compute the standard deviation of the sample means
...
plot(n_values, means_m_values, 
     ...
     lty = "dashed",
     col = "orange",
     main = "Impact of sample size on the mean of sampling means",
     ylim = c(my_min, my_max), 
     ylab = paste0("Mean of the sampling mean (", r, " repetitions)"),
     xlab = "Sample size (n)",
     las = 1,
     panel.first = c(abline(h = mu, col = "blue", lwd = 1), 
                   grid())
     )
