...
## Démo 4: impact de la taille d'échantillon


Nous fournissons ci-dessous le code pour refaire le calcul de la moyenne d'échantillon,  en maintenant le même nombre de répétitions ($r=`r r`$), mais en faisant varier la taille d'échantillon $n$, qui prendra successivement les valeurs $1$, $2$, $4$, $8$, $16$, $32$, $64$.

**Astuce: ** pour automatiser l'analyse avec une série de valeurs de $n$, vous nous utilisons une boucle `for`. 

Le bloc de code ci-dessous contient la solution. 


Pour chaque valeur de $n$, 

- nous dessinons l'histogramme, 
- nous calculons la moyenne (`mean()` la variance (`var()`) et l'écart-type (`sd()`) des moyennes d'échantillons
...
Interprétez le résultat : quelle est la relation entre
