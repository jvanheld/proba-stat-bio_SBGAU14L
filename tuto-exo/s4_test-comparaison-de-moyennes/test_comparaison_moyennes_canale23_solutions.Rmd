...
my_t <- function (x, y, ...) {
 ...
 t_result_list <- list(
   ...
   t_result$estimate,
   side = t_result$alternative,
   method = t_result$method)
 return(as.data.frame(t_result_list))
}
...
my_t <- function (values, groups, ...) {
 ...
 t_result_vector <- c(
   ...
   conf.int = t_result$conf.int,
   t_result$estimate,
   d = unname(t_result$estimate[2] - t_result$estimate[1]))
...
proteome_t$E <- proteome_t$p * nrow(proteome_t)
...
```{r volcano_plot, fig.width=10, fig.height=10, out.width="90%", fig.cap="Volcano plot of the proteome differential analysis. "}
#### Volcano plot ####
y <- -log10(proteome_t$p)
plot(proteome_t$d, 
     y, 
     main = "Volcano plot of the proteome differential analysis\nCanale (2023)",
     xlab = "Effect size = m(Liver) - m(HCC)",
     ylab = "-log10(P-value)",
     col = densCols(proteome_t$d, y),
     las =  1,
     panel.first = grid())
...
threshold_d <- 2    ## Threshold on effect size (=log2(FC))
...
abline(v = c(-threshold_d, +threshold_d), col="darkgreen")

## Recolor significant proteins
selected_up <- rownames(subset(proteome_t, (E < threshold_E) & (d > threshold_d))) 
selected_down <- rownames(subset(proteome_t, (E < threshold_E) & (d < -threshold_d))) ## Conservative Bonferoni threshold
lines(proteome_t[selected_up, "d"], 
      -log10(proteome_t[selected_up, "p"]), 
      type = "p", pch=20, col = "#00AA00", cex=0.5)
lines(proteome_t[selected_down, "d"], 
